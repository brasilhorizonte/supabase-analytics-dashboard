-- Migration: Migrate questions_daily e questions_by_user de user_daily_usage para terminal_events
-- Apply to: Horizon Terminal Access (llqhmywodxzstjlrulcw)
--
-- Motivo: O sistema legado de question_count (user_daily_usage) foi removido.
-- Perguntas agora sao contadas a partir de terminal_events (event_name = 'terminal_message_send').
-- Dados historicos de user_daily_usage ficam como fallback para datas antes de 2026-03-24.
--
-- Mudancas:
--   questions_daily: UNION de user_daily_usage (historico) + terminal_events (atual)
--   questions_by_user: Nova secao baseada em terminal_events (nunca existia na RPC)
...
BEGIN
  SELECT jsonb_build_object(
    ...
    -- Token stats from terminal_events (agent answer_done)
    'token_stats', (
    ...
    -- Token breakdown by response mode (Deep vs Fast vs Pro)
    'token_by_mode', (
      SELECT coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb)
      FROM (
        SELECT
    ...
    -- Questions daily: terminal_events (terminal_message_send) com fallback historico de user_daily_usage
    'questions_daily', (
      SELECT coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb)
      FROM (
        SELECT day, SUM(total_questions) as total_questions,
               SUM(active_users) as active_users,
               CASE WHEN SUM(active_users) > 0
                 THEN round(SUM(total_questions)::numeric / SUM(active_users), 1)
                 ELSE 0
               END as avg_per_user
        FROM (
          -- Dados historicos de user_daily_usage (antes de terminal_events existir)
          SELECT usage_date as day,
                 sum(question_count) as total_questions,
                 count(DISTINCT user_id) as active_users
          FROM public.user_daily_usage
          WHERE usage_date < '2026-02-10'::date  -- antes do tracking via terminal_events
          GROUP BY usage_date
          UNION ALL
          -- Dados atuais de terminal_events (terminal_message_send)
          SELECT date_trunc('day', event_ts AT TIME ZONE 'America/Sao_Paulo')::date as day,
                 count(*) as total_questions,
                 count(DISTINCT user_id) as active_users
          FROM public.terminal_events
          WHERE event_name = 'terminal_message_send'
            AND date_trunc('day', event_ts AT TIME ZONE 'America/Sao_Paulo')::date >= '2026-02-10'::date
          GROUP BY day
        ) combined
        GROUP BY day
        ORDER BY day ASC
    ...
    -- Questions by user: total de perguntas por usuario (terminal_events)
    'questions_by_user', (
      SELECT coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb)
      FROM (
        SELECT
          te.user_id,
          coalesce(u.email, te.user_id::text) as email,
          count(*) as total_questions
        FROM public.terminal_events te
        LEFT JOIN auth.users u ON u.id = te.user_id
        WHERE te.event_name = 'terminal_message_send'
