-- Airton Analytics RPC v2 (2026-05-12)
-- Janela temporal parametrizada + admin filter via usage_events_clean (já criada
-- em 20260503_bh_engagement_v2_admin_filter.sql).
--
-- Fontes:
--   - usage_events_clean: eventos companion_* (web) + gemini_token_usage (tokens)
--   - companion_messages (source='telegram'): mensagens via Telegram (não emitem usage_events)
--   - companion_threads: stats de threads + last_user_source (web/telegram)
--
-- Admins excluídos via JOIN auth.users em queries que tocam companion_messages/threads
-- (replicando o pattern do usage_events_clean para essas tabelas).
...
BEGIN
  ...
  -- Telegram-side bases — JOIN auth.users para excluir admins
  cm AS (
    SELECT m.*
    FROM companion_messages m
    LEFT JOIN auth.users u ON u.id = m.user_id
    WHERE m.created_at BETWEEN p_from AND p_to
      AND (u.email IS NULL OR NOT (u.email = ANY(v_admin_emails)))
  ),
  ct AS (
    SELECT t.*
    FROM companion_threads t
    LEFT JOIN auth.users u ON u.id = t.user_id
    WHERE (u.email IS NULL OR NOT (u.email = ANY(v_admin_emails)))
  ),
  -- ─── Overview KPIs ───────────────────────────────────────────────────
  tokens_agg AS (
    SELECT
  ...
  -- ─── Tool calls by tool_name (using companion_tool_call which has tool_name in properties) ─
  tool_top AS (
  ...
  SELECT jsonb_build_object(
    ...
    'meta', jsonb_build_object(
      'from', p_from,
      'to', p_to,
      'admins_excluded', array_length(v_admin_emails, 1),
...
GRANT EXECUTE ON FUNCTION public.get_analytics_data_airton_v2(timestamptz, timestamptz) TO anon, authenticated, service_role;
