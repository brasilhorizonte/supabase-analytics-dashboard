-- =====================================================================
-- 20260817_dashboard_overhaul_bh.sql
-- Projeto: brasilhorizonte (dawvgbopyemcayavcatd)
--
-- Overhaul das RPCs do dashboard (2026-08-17):
--   Bloco 1: get_analytics_data_v2_impl — remove 8 secoes mortas/caras
--   Bloco 2: NEW get_notification_analytics_v2(p_from, p_to) — janela parametrizada
--   Bloco 3: get_analytics_data_bh_utm_v2 — remove campanha 50OFF (expirada)
--   Bloco 4: statement_timeout = '30s' nas RPCs restantes
--
-- Fonte da verdade: definicoes vivas no banco (pg_get_functiondef),
-- NAO os arquivos de migration anteriores deste repo.
-- =====================================================================

-- =====================================================================
-- Bloco 1 — get_analytics_data_v2_impl
-- ---------------------------------------------------------------------
-- 2026-08-17: removidas 8 secoes do jsonb_build_object:
--   * top_tickers_market, sector_distribution — liam brapi_quotes
--     (market data irrelevante para o dashboard)
--   * ticker_by_feature, ticker_trend_daily, feature_usage_trend,
--     ticker_ranking, user_ticker_usage, user_ticker_detail — essas 6
--     sao sobrescritas no merge da Edge Function pelos resultados de
--     get_analytics_data_bh_tickers_v3 (trabalho 100% desperdicado;
--     varias eram all-time e caras)
-- O wrapper get_analytics_data_v2 (auth admin/service_role) NAO muda.
-- =====================================================================

CREATE OR REPLACE FUNCTION public.get_analytics_data_v2_impl(p_from timestamp with time zone DEFAULT (now() - '30 days'::interval), p_to timestamp with time zone DEFAULT now())
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- =====================================================================
-- Bloco 2 — NEW get_notification_analytics_v2(p_from, p_to)
-- ---------------------------------------------------------------------
-- Versao parametrizada de get_notification_analytics() (v1 intocada).
-- Secoes event-based (alert_notifications / telegram_links_daily /
-- partes event-based do notification_funnel) respeitam a janela
-- p_from..p_to. Snapshots de estado (telegram_overview,
-- notification_prefs_summary, notification_type_popularity e os
-- contadores de estado do funnel) continuam all-time.
-- Mesmas chaves de retorno da v1 — frontend nao muda.
-- =====================================================================

CREATE OR REPLACE FUNCTION public.get_notification_analytics_v2(p_from timestamp with time zone DEFAULT (now() - '90 days'::interval), p_to timestamp with time zone DEFAULT now())
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET statement_timeout TO '30s'
...
BEGIN
  SELECT json_build_object(
    -- 1. Notifications by type (daily, window p_from..p_to)
    ...
    -- 2. Notifications delivery overview (window p_from..p_to)
    ...
    -- 4. Top tickers by notification volume (window p_from..p_to)
    ...
    -- 5. Telegram overview (snapshot de estado — all-time)
    'telegram_overview', (
      SELECT row_to_json(r)
      FROM (
        SELECT
    ...
    -- 6. Telegram links daily (new links over time, window p_from..p_to)
    'telegram_links_daily', (
      ...
      FROM (
        ...
        WHERE linked_at IS NOT NULL
          AND linked_at BETWEEN p_from AND p_to
        GROUP BY day
        ORDER BY day DESC
      ) r
    ),
    ...
    --    (contadores event-based respeitam a janela; contadores de
    --     estado — total_users, prefs enabled, telegram links — sao snapshot)
    'notification_funnel', (
      SELECT row_to_json(r)
      FROM (
        SELECT
          (SELECT COUNT(*) FROM auth.users) AS total_users,
          (SELECT COUNT(DISTINCT user_id) FROM alert_notifications WHERE created_at BETWEEN p_from AND p_to) AS users_with_notifications,
          (SELECT COUNT(DISTINCT user_id) FROM alert_notifications WHERE read = true AND created_at BETWEEN p_from AND p_to) AS users_who_read,
          ...
          (SELECT COUNT(DISTINCT user_id) FROM alert_notifications WHERE telegram_delivered = true AND created_at BETWEEN p_from AND p_to) AS users_telegram_delivered
      ) r
    ),

    -- 8. Notification preferences summary (snapshot de estado — all-time)
    ...
    -- 9. Notification type popularity (from preferences — snapshot all-time)
...
-- Grants: mesmo padrao da v1 (authenticated + service_role; nunca anon)
REVOKE ALL ON FUNCTION public.get_notification_analytics_v2(timestamptz, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_notification_analytics_v2(timestamptz, timestamptz) TO authenticated, service_role;

-- =====================================================================
-- Bloco 3 — get_analytics_data_bh_utm_v2
-- ---------------------------------------------------------------------
-- 2026-08-17: removidas as 3 secoes da campanha 50OFF (expirada em
-- 2026-05-03): usage_utm_50off_summary, usage_utm_50off_daily,
-- usage_utm_50off_top_content — e as constantes promo_start/promo_end
-- que so elas usavam. Demais secoes preservadas byte a byte.
-- =====================================================================

CREATE OR REPLACE FUNCTION public.get_analytics_data_bh_utm_v2(p_from timestamp with time zone DEFAULT (now() - '90 days'::interval), p_to timestamp with time zone DEFAULT now())
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
  ...
  result := jsonb_build_object(
    ...
    'usage_utm_daily', (
      ...
      FROM (
        ...
        FROM public.usage_events
        WHERE event_ts BETWEEN p_from AND p_to AND utm_source IS NOT NULL
    ...
    'usage_utm_by_content', (
      ...
      FROM (
        SELECT
          ...
          count(*) FILTER (WHERE event_name = 'payment_succeeded') AS payments
        FROM public.usage_events
        WHERE event_ts BETWEEN p_from AND p_to AND utm_content IS NOT NULL
    ...
    'usage_utm_funnel_by_source', (
      ...
      FROM (
        WITH attributed_sessions AS (
          ...
          FROM public.usage_events
          WHERE event_ts BETWEEN p_from AND p_to AND utm_source IS NOT NULL AND session_id IS NOT NULL
        )
        SELECT
          a.utm_source,
          count(DISTINCT a.session_id) AS sessions,
          count(DISTINCT a.session_id) FILTER (
            WHERE EXISTS (SELECT 1 FROM public.usage_events e WHERE e.session_id = a.session_id AND e.event_name = 'auth_login')
          ) AS sessions_with_login,
          count(DISTINCT a.session_id) FILTER (
            WHERE EXISTS (SELECT 1 FROM public.usage_events e WHERE e.session_id = a.session_id AND (e.event_name LIKE 'paywall%' OR e.event_name = 'paywall_block'))
          ) AS sessions_with_paywall,
          count(DISTINCT a.session_id) FILTER (
            WHERE EXISTS (SELECT 1 FROM public.usage_events e WHERE e.session_id = a.session_id AND e.event_name = 'payment_succeeded')
          ) AS sessions_with_payment
        FROM attributed_sessions a
        GROUP BY a.utm_source HAVING count(DISTINCT a.session_id) >= 3
...
-- =====================================================================
-- Bloco 4 — statement_timeout nas RPCs que ficaram de fora
-- ---------------------------------------------------------------------
-- Assinaturas confirmadas via pg_get_function_identity_arguments
-- (2026-08-17). get_geo_profiles() nao tem argumentos.
-- =====================================================================

ALTER FUNCTION public.get_analytics_data_bh_pool_v2(timestamptz, timestamptz) SET statement_timeout = '30s';
ALTER FUNCTION public.get_analytics_data_bh_tickers_v3(timestamptz, timestamptz, boolean, text[], integer) SET statement_timeout = '30s';
ALTER FUNCTION public.get_analytics_data_airton_telegram_v1(timestamptz, timestamptz) SET statement_timeout = '30s';
ALTER FUNCTION public.get_analytics_data_airton_whatsapp_v1(timestamptz, timestamptz) SET statement_timeout = '30s';
ALTER FUNCTION public.get_geo_profiles() SET statement_timeout = '30s';
