-- Migration: BH revenue fixes + new tracking sections
-- Date: 2026-04-18
-- Fixes revenue_by_plan (ialocador R$59,90, ianalista R$39,90), adds trial tracking,
-- active_subscribers_daily/new_subscribers_daily time series, portfolio activity,
-- alert_rules analytics, activation funnel, email/whatsapp logs, cvm activity,
-- tab_view breakdown, and expands feature_usage LIMIT 10 -> 20.
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'iacoes_overview', (
      SELECT jsonb_build_object(
        'total_views', (SELECT count(*) FROM public.iacoes_page_views WHERE event_type = 'pageview' OR event_type IS NULL),
        ...
        'views_today', (SELECT count(*) FROM public.iacoes_page_views WHERE created_at >= (now() AT TIME ZONE 'America/Sao_Paulo')::date AND (event_type = 'pageview' OR event_type IS NULL)),
        'sessions_today', (SELECT count(DISTINCT session_id) FROM public.iacoes_page_views WHERE created_at >= (now() AT TIME ZONE 'America/Sao_Paulo')::date)
      )
    ),
    'iacoes_daily', (
      SELECT coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb)
    ...
    'iacoes_devices', (
      ...
      FROM (
        ...
        FROM public.iacoes_page_views WHERE device_type IS NOT NULL
    ...
    'iacoes_browsers', (
      ...
      FROM (
        ...
        FROM public.iacoes_page_views WHERE browser IS NOT NULL
    ...
    'iacoes_os', (
      ...
      FROM (
        ...
        FROM public.iacoes_page_views WHERE os IS NOT NULL
  ...
  result := result || jsonb_build_object(
    'iacoes_conversion_funnel', (
      SELECT jsonb_build_object(
        'iacoes_views', (SELECT count(*) FROM public.iacoes_page_views WHERE event_type = 'pageview' OR event_type IS NULL),
        'cta_clicks', (SELECT count(*) FROM public.iacoes_page_views WHERE event_type = 'cta_click'),
