-- Airton Analytics RPC v2 — admin filter + tool event drift fix (2026-05-13)
--
-- Bugs corrigidos:
--
-- 1. Hardcoded admin email list ficava stale e divergia do filtro do
--    usage_events_clean (que usa profiles.is_admin). Resultado: admins novos
--    (ex.: joao.lasmar@brasilhorizonte.com.br linkado em 05/05) não eram
--    excluídos das métricas Telegram e inflavam tg_unique_users / threads.
--    Fix: NOT EXISTS profiles is_admin nas CTEs `cm` e `ct`, alinhando com o
--    pattern já vivo em usage_events_clean.
--
-- 2. Drift de event_name: tracker emite tanto `companion_tool_call` quanto
--    `companion_tool_called`. `msg_agg.tool_calls` e `funnel.tool_called` já
--    incluíam ambos; só `tool_top` e `tool_calls_daily` filtravam o singular.
--    Fix: ambos os blocos agora usam IN (...).
...
BEGIN
  ...
  -- Telegram-side bases — admin filter via profiles.is_admin (pattern canônico,
  -- alinhado com usage_events_clean). NOT EXISTS evita JOIN explícito com
  -- auth.users e captura admins dinamicamente.
  cm AS (
    SELECT m.*
  ...
  -- Aceita ambos os event_names emitidos pelo tracker:
  -- `companion_tool_call` (canônico, com action success/error/empty) e
  -- `companion_tool_called` (variante adicionada em 2026-05-12).
  tool_top AS (
  ...
  SELECT jsonb_build_object(
    ...
    'meta', jsonb_build_object(
      'from', p_from,
      'to', p_to,
      'admins_excluded_via', 'profiles.is_admin',
      'source', 'usage_events_clean + companion_messages + companion_threads',
      'rpc_version', 'airton_v2.1_admin_filter_fix_20260513'
    )
  )
  INTO v_result;
...
GRANT EXECUTE ON FUNCTION public.get_analytics_data_airton_v2(timestamptz, timestamptz)
  TO anon, authenticated, service_role;

-- Reload PostgREST schema cache para garantir que /rpc/... pega a nova definição
